...
public class Exam6 {
	...
	/*
	예외를 강제로 발생 시키키 throw
	예를 들면 int age = -1; 실제는 예외가 아니다.
	하지만 경우에 따라 나이는 -1살은 절대 있을수가 없으므로 강제로 예외로 인식시킬 필요가 있다.
	몸무게의 경우에도 500kg는 절대 있을수가 없는 값이므로 예외로 인식시킬 필요가 있다.
	
	이럴경우 해결 방법은 예외 클래스를 직접 정의하여 사용하는 것이다.
	사용자 정의 예외 클래스를 만들어보면 다른 예외들의 동작 메커니즘도 자연스럽게 이해를 할 수 있다.
	*/
