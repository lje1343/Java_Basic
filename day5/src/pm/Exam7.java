...
public class Exam7 {
	...
	public static void main(String[] args) {
		...
		자바는 객체지향언어로 클래스, 추상클래스, 인터페이스와 같은 객체 지향적 문법
		요소를 사용하여 프로그램을 구성하게 된다.
		
		자바를 제대로 사용하려면 이러한 문법 요소를 먼저 이해하는 것이 아주 중요하다.
		
		1. 클래스의 개념
			객체는 object 사용할 수 있는 실체를 의미하고 클래스는 그 객체를 만드는
			설계도라고 볼 수 있다. 따라서 한개의 클래스로 여러개의 객체를 만들 수 있다.
			...
			구조체 : 배열은 같은 자료형만 묶을 수 있으므로 여러 형태의 데이터를 한개의 
				배열로는 관리할 수 없다. 배열의 단점을 보완하기 위해 사용되는 자료형
			클래스 : 구조체는 데이터만 관리할 수 있다. 여기에 어떤 기능등(메서드)을
					추가한것이 바로 클래스이다.
			...
			크게 보면 2개 (클래스와 인터페이스)
			작게 보면 3개 일반클래스, 추상클래스, 인터페이스의 객체 지향 문법 요소를 사용한다.
			
			클래스 작성의 기본 규칙들>
			
			클래스명과 파일명은 반드시 일치해야만 한다.
			클래스의 밖에 올 수 있는 것은 package import 또다른 클래스 선언
			
			접근 지정자 public은 한개의 .java 파일안에서 최대1개의 클래스에만 붙일 수 있다.
			
			객체와 인스턴스> (인스턴스보다는 객체 표현이 더 많이 쓰인다.)
