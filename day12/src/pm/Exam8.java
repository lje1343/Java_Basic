...
public class Exam8{
		/*
		 ...
		 3. 앞에서 정의한 클래스를 이용해서 Runnable 객체를 생성한다.
		 	문제는 Runnable 객체의 내부는 start() 메서드가 존재하지 않는다.
		 	그렇기 때문에 start()를 가진 Thread 객체를 생성해야 한다.
		 	
		 	다만 구현한 run() 메서드 자체는 Runnable 객체가 갖고 있으므로 Thread 객체를
		 	생성할때 Runnable 객체를 생성자의 매개변수로 넘겨준다.
		 	
		 	이렇게 객체를 생성하면 생성된 Thread 객체 내부의 run() 메서드는 생성자 매개변수로
		 	넘어온 Runnable 객체 내부의 run()으로 대체된다.
		 */
