...
public class Exam6 extends Thread{
	/*
	 ...
	 2. 1단계 - Runnable 인터페이스를 구현한 Runnable 객체를 생성한다.
	...
	start() 메서드를 호출해야 한다는 것을 명심하자
	주의할 점은 start() 메서드로 한번 실행된 객체는 재사용할 수 없다는 것이다.
...
/*
    ...
    프로그램이 처음 실행될 때 이미 main 쓰레드가 실행되고 있으므로 이 2개의 작업을 동시에 실행하기
    위해서는 적어도 하나 이상의 쓰레드를 추가로 생성하여 실행해야 한다.
 
  Thread 클래스를 상속받아서 클래스를 정의했고 
    객체를 생성하여 start() 메서드로 실행하였다.
  main 쓰레드와 smiFileThread 를 사용하여 각각 비디오 번호와 자막번호를 출력했다.
  
   참고로 멀티 쓰레드는 독립적으로 실행되기 때문에 먼저 start() 메서드로 호출이 되었다 해도
   나중에 실행된 쓰레드 보다 늦게 실행될 수 있다.
   이런 이류로 자막 번호가 항상 비디오 번호 뒤에 나오도록 쓰레드에 Thread.sleep(10)을 추가해서
  0.01초 늦게 출력되도록 하였다.
 */