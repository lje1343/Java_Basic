...
public class Exam5 {
	/*
	 ...
	 저장된 형태로 캐스팅해야한다. 필드 자체가 Object 타입이기 때문에 
	 get() 메서드로 가져오는 타입 또한 항상 Object타입이다. 따라서 Apple객체를 저장했을 때는
	 get() 메서드로 가져온 Object을 Apple 타입 혹은 Pencil 객체를 저장했을때는
	 가져온 Object 타입을 각각 Pencil타입으로 캐스팅 해야 한다.
	 
	 하지만 실수로 실제로는 Apple 객체인데 가져올때는 Pencil 객체로 캐스팅을 하게되면
	 잘 못 캐스팅했을 때 발생하는 예외인 ClassCastException 는 실행 예외이기 때문에
...
/*
 ...
 선언하면 모든 타입을 저장할수는 있다. 문제 없다. 다만... 객체를 get() 메서드로
