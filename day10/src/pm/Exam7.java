...
public class Exam7 {
	...
	public static void main(String[] args) {
		...
		이것은 마치 자바가 catch()라는 이름의 메서드를 호출하는것과 비슷하다.
		만약 자바가 생성해서 넘겨준 객체 타입을 catch 블록이 처리할 수 없을때 
		즉 해당 객체를 받을 catch() 블록이 존재하지 않으면 예외처리가 되지 않는다.
		
		절대 메서드는 아니다... 하지만 메서드가 다양한 입력 매개변수 타입으로 오버로딩 될 수 있는 것처럼
		catch() 블록도 예외 타입에 따라 여러개를 포함 할 수 있다.
		...
		try {
			...
			System.out.println("숫자는 0으로 나눌 수 없습니다.");
		} catch (NumberFormatException e) {	// 위에서 걸리면 다음 예외들은 점프해버린다.
		...
		이것은 또 마치 if-else if-else 구문에서 조건식을 위에서부터 검사하는 것과 같다.
		 ...
		 둘중 어떤 예외가 발생해도 모두 하나의 예외처리로 해결할 수 있다.
		...
		/* 
		 ...
		 리소스 해제는 더 이상 사용하지 않는 자원을 반납하는 것을 의미한다.
